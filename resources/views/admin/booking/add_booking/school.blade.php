...
<form class="row g-3 needs-validation custom-input" action="{{ route('bookings.school.form') }}" method="POST">
    @csrf
        ...
        <div class="row g-3">
                ...
                <div class="input-group">
                    <div class="input-group-text">
                        <input class="form-check-input mt-0" type="radio" name="entry_type" value="school"
                            {{ old('entry_type', 'school') == 'school' ? 'checked' : '' }}>
                    </div>
                ...
                <div class="input-group">
                    <div class="input-group-text">
                        <input class="form-check-input mt-0" type="radio" name="entry_type" value="pre_school"
                            {{ old('entry_type') == 'pre_school' ? 'checked' : '' }}>
                    </div>
            ...
            <div class="col-xxl-4 col-sm-6">
                ...
                <select class="form-select @error('language') is-invalid @enderror" name="language" required>
                    ...
                    <option value="french" {{ old('french') == 'english' ? 'selected' : '' }}>French</option>
                </select>
                @error('language')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="col-xxl-4 col-sm-6">
                ...
                <input class="form-control @error('alayacare_service_code') is-invalid @enderror"
                    ...
                    placeholder="Enter service code" value="{{ old('alayacare_service_code') }}" required>
            ...
            <div class="col-xxl-4 col-sm-6">
                ...
                <input class="form-control @error('private_room_area') is-invalid @enderror" name="private_room_area"
                    type="text" placeholder="Ente private room/area" value="{{ old('private_room_area') }}">
            ...
            <div class="col-xxl-4 col-sm-6">
                ...
                <input class="form-control @error('specific_time') is-invalid @enderror" name="specific_time"
                    type="text" placeholder="Ente specific time of day" value="{{ old('specific_time') }}">
                @error('specific_time')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-xxl-4 col-sm-6">
                <label class="form-label" for="validationCustom-b">Area of Town<span
                        class="txt-danger">*</span></label>
                <select class="form-select @error('area_of_town') is-invalid @enderror" name="area_of_town" required>
                ...
                <div class="row g-3">
                    <div class="col-xxl-6 col-sm-6">
                        ...
                        <select class="form-select @error('units_from') is-invalid @enderror" id="validationCustom04"
                            required="" name="units_from" id="units_from">
                            <option value="">Select Units</option>
                            @foreach (config('booking.unit_from') as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('units_from') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-xxl-6 col-sm-6">
                        <label class="form-label" for="validationCustom04">Units to<span
                                class="txt-danger">*</span></label>
                        <select class="form-select @error('units_to') is-invalid @enderror" id="validationCustom04"
                            required="" name="units_to" id="units_to">
                            <option value="">Select Units</option>
                            @foreach (config('booking.unit_to') as $key => $label)
                                <option value="{{ $key }}" {{ old('units_to') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <select class="form-select @error('therapist') is-invalid @enderror therapist"
                    ...
                    @foreach ($therapists as $therapist)
                        <option value="{{ $therapist->id }}"
                            {{ old('therapist') == $therapist->id ? 'selected' : '' }}>
                            {{ $therapist->name }}
                        </option>
                    @endforeach
                </select>
                    ...
                    <div class="col-xxl-4 col-sm-6">
                        <div class="input-group">
                            <div class="input-group-text">
                                <input class="form-check-input mt-0" name="inform_to[]" type="checkbox"
                                    value="educator_only"
                                    {{ in_array('educator_only', old('inform_to', [])) ? 'checked' : '' }}>
                            </div>
                        ...
                        <div class="input-group">
                            <div class="input-group-text">
                                <input class="form-check-input mt-0" name="other_app_scheduling_info[]"
                                    ...
                                    {{ in_array('am', old('other_app_scheduling_info', [])) ? 'checked' : '' }}>
                            </div>
                        ...
                        <div class="input-group">
                            <div class="input-group-text">
                                <input class="form-check-input mt-0" name="other_app_scheduling_info[]"
                                    ...
                                    {{ in_array('pm', old('other_app_scheduling_info', [])) ? 'checked' : '' }}>
                            </div>
            ...
            <div class="col-xxl-4 col-sm-6">
                ...
                <select class="form-select select2 @error('days_off_week') is-invalid @enderror"
                    ...
                    @foreach (['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                        <option value="{{ $day }}"
                            {{ in_array($day, old('days_off_week', [])) ? 'selected' : '' }}>
                            {{ $day }}
                        </option>
                    @endforeach
                </select>
